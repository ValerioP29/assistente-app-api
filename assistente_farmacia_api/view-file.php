...
// Determina se il file è visualizzabile nel browser
$viewable_types = [
    'application/pdf',
    'image/jpeg',
    'image/jpg',
...
$is_viewable = in_array($file_info['mime_type'], $viewable_types);

// Imposta headers per la visualizzazione
header('Content-Type: ' . $file_info['mime_type']);
header('Content-Length: ' . $file_info['size']);
header('Cache-Control: public, max-age=3600'); // Cache per 1 ora
header('Expires: ' . gmdate('D, d M Y H:i:s \G\M\T', time() + 3600));

// Se il file non è visualizzabile, forza il download
if (!$is_viewable) {
