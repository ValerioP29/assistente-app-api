...
if( ! $reminder_id || ! $product_name || ! $expiry_date || ! $alerts ){
	echo json_encode([
		'code'    => 400,
		'status'  => false,
		'error'   => 'Bad Request',
...
// Gestione del file caricato
$file_path = $existing_reminder['file']; // Mantieni il file esistente di default
if ($uploaded_file) {
...
$updated = update_reminder_expiry( $reminder_id, $user['id'], $reminder_data );
...
echo json_encode([
	'code'    => 200,
	'status'  => true,
	'message' => 'Promemoria scadenza aggiornato con successo',
]); 